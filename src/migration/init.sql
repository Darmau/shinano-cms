...
CREATE TYPE FOLDER AS ENUM('photography', 'picture', 'cover');
...
CREATE TABLE
  image (
    ...
    "created_at" TIMESTAMPTZ DEFAULT NOW(),
    "storege_key" UUID NOT NULL DEFAULT uuid_generate_v4 (),
    UNIQUE ("date", "file_name", "format")
  );

CREATE INDEX ON image ("date", "file_name", "format");

CREATE UNIQUE INDEX ON image ("storege_key");
